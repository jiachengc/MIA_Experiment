...
def mmd_rbf(X, Y, sigma=1.0):
    """计算 RBF 核的 MMD"""
    n, m = len(X), len(Y)
    XX = torch.cdist(X.unsqueeze(1), X.unsqueeze(1), p=2).pow(2) / (2 * sigma ** 2)
    YY = torch.cdist(Y.unsqueeze(1), Y.unsqueeze(1), p=2).pow(2) / (2 * sigma ** 2)
    XY = torch.cdist(X.unsqueeze(1), Y.unsqueeze(1), p=2).pow(2) / (2 * sigma ** 2)
    
    kernel_XX = torch.exp(-XX).sum() / (n * n)
    kernel_YY = torch.exp(-YY).sum() / (m * m)
    kernel_XY = torch.exp(-XY).sum() / (n * m)
    
    mmd2 = kernel_XX + kernel_YY - 2 * kernel_XY
    return mmd2
# 辅助函数
def prepare_model_for_dp(model, noise_multiplier=0.4, max_grad_norm=1.0):
    """
    为模型添加 opacus 差分隐私支持。
    """
    privacy_engine = PrivacyEngine()
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01)  # 最小化优化器以初始化钩子
    dummy_loader = DataLoader([], batch_size=32)  # 占位符
...
def compute_gradient_norm(model, data, target, criterion):
    """
    计算模型在给定数据上的梯度范数，支持 opacus 的 grad_sample。
    """
    model.zero_grad()
    output = model(data)  # 前向传播以触发 opacus 钩子
    loss = criterion(output, target)
    loss.backward()  # opacus 应跟踪 grad_sample
    grad_norm = 0
    for param in model.parameters():
        if hasattr(param, 'grad_sample') and param.grad_sample is not None:
            grad_norm += param.grad_sample.norm() ** 2  # 使用 opacus grad_sample
        elif param.grad is not None:
            grad_norm += param.grad.norm() ** 2  # 回退到常规梯度
    return torch.sqrt(grad_norm)
def optimize_guesses(scores, canary_idx, m, max_iter=1000):
    """
    使用逻辑回归和迭代优化动态调整猜测策略。
    """
    # 确保 scores 是 2D 数组，形状为 [n_samples, 1]
    scores_2d = scores.detach().cpu().numpy().reshape(-1, 1)  # 转换为 [n_samples, 1]
    canary_idx_1d = canary_idx.detach().cpu().numpy()  # 确保是 1D 数组

    G = LogisticRegression(max_iter=1000)
    train_idx = np.arange(min(100, m))  # 使用前 100 个样本训练，或全部如果 m < 100
    G.fit(scores_2d[train_idx], canary_idx_1d[train_idx])

    T_probs = G.predict_proba(scores_2d)[:, 1]
    T = (T_probs > 0.5).astype(int) * 2 - 1  # 转换为 {-1, 1}

    v_initial = sum(max(0, T[i] * canary_idx_1d[i]) for i in range(m))
    for _ in range(max_iter):
        v_current = sum(max(0, T[i] * canary_idx_1d[i]) for i in range(m))
        uncertain_idx = np.where(abs(T_probs - 0.5) < 0.1)[0]  # 找到不确定样本
        for idx in uncertain_idx:
            T[idx] = np.random.choice([-1, 1])  # 随机调整不确定样本的猜测
...
def _audit(
    ...
    #     """
    #     计算差分隐私审计的 p-value，使用更紧致的统计界限。
        
    #     Args:
    #         m (int): 数据集总大小
    #         r (int): 测试样本数
    #         v (int): 正确猜测数
    #         eps (float): 隐私预算 epsilon
    #         delta (float): 差分隐私的 delta 参数
    ...
    #     q = 1 / (1 + math.exp(-eps))  # 攻击成功概率
    #     lambda_ = r * q * (1 - (r - 1) / m)  # 泊松二项分布参数
    #     beta = poisson.sf(v - 1, lambda_)  # P[Poisson(lambda) >= v]

    #     alpha = 0
    #     sum_prob = 0
    #     for i in range(1, min(v + 1, int(m * delta) + 1)):
    #         sum_prob += poisson.pmf(v - i, lambda_)
    #         if sum_prob > i * alpha:
    #             alpha = sum_prob / i

    #     p = beta + alpha * delta * m
    #     return min(p, 1.0)
    ...
    for shadow_idx in range(0, 32):
        ...
        if score_type == "global_loss":
            ...
            print(f"Number of samples in data_pipe: {len(list(train_data_full_pipe))}")
            ...
            print(f"Number of samples in data_pipe: {len(list(train_data_full_pipe))}")
            # 检查 nan 和 inf
            if torch.isnan(init_pred_).any() or torch.isinf(init_pred_).any() or torch.isnan(final_pred_).any() or torch.isinf(final_pred_).any():
                raise ValueError("Predictions contain nan or inf values")
            
            # 限制数值范围
            print("Predicting logits and evaluating full training data")
            init_pred_ = torch.clamp(init_pred_, min=-1e6, max=1e6)
            final_pred_ = torch.clamp(final_pred_, min=-1e6, max=1e6)
            
            # 计算 loss_diff 和 logit_diff
            criterion = torch.nn.CrossEntropyLoss(reduction='none')
            loss_diff_ = criterion(final_pred_, attack_ys) - criterion(init_pred_, attack_ys)
            logit_diff_ = torch.norm(final_pred_ - init_pred_, p=2, dim=1)
            
            # 结合特征
            w1, w2 = 0.7, 0.3  # 可优化权重
            logit_diff_canary = logit_diff_[canary_indices]
            loss_diff_canary = loss_diff_[canary_indices]
            combined_scores = w1 * loss_diff_canary + w2 * logit_diff_canary
            ...
            in_features = combined_scores[in_idx]
            out_features = combined_scores[out_idx]
            # 计算 MMD
            sigma = args.sigma  # 带宽参数
            mmd_value = mmd_rbf(in_features, out_features, sigma)
            mmd_max = 1.0  # 可根据数据调整
            mmd_scale = mmd_value / mmd_max if mmd_value > 0 else 0.0
            
            # 动态加权得分
            lambda_param = args.lambdas  # 加权因子
            weights = 1 + lambda_param * (1 - mmd_scale) * (canary_idx == 1).float()
            sorted_scores = combined_scores * weights
            
            # 提取 canary 样本的得分（保持原逻辑）
        ...
        # 假设已有变量：args, data_generator, canaries_dir, output_dir, shadow_idx, shadow_membership_mask, attack_ys, canary_indices
        # if score_type == "global_loss":
        #     initial_model = _build_model(num_classes=10)
        #     final_model = torch.load(f"{output_dir}/shadow/{shadow_idx}/model.pt")
        ...
        #     # 计算损失差异
        #     criterion = torch.nn.CrossEntropyLoss(reduction='none')
        #     init_loss_ = criterion(init_pred_, attack_ys)
        #     final_loss_ = criterion(final_pred_, attack_ys)
        #     loss_diff_ = init_loss_ - final_loss_
            
        #     # 计算置信度变化
        #     init_conf_ = torch.softmax(init_pred_, dim=1).max(dim=1)[0]
        #     final_conf_ = torch.softmax(final_pred_, dim=1).max(dim=1)[0]
        #     conf_diff_ = final_conf_ - init_conf_
            
        #     # 计算 logit 变化
        #     logit_diff_ = torch.norm(final_pred_ - init_pred_, p=2, dim=1)
            
        #     # 融合多维特征
        #     w1, w2, w3 = 1.0, 0, 1  # 可调整的权重
        #     sorted_scores_full = w1 * loss_diff_ + w2 * conf_diff_ + w3 * logit_diff_
        #     sorted_scores = sorted_scores_full[canary_indices]
        #     # 保存得分（保持原始逻辑）
        #     np.save(f"{output_dir}/sorted_scores.npy", sorted_scores.detach().cpu().numpy())
        #     # print(f"sorted_scores_full shape: {sorted_scores_full.shape}")
        #     # print(f"sorted_scores shape: {sorted_scores.shape}")
        #     # print(f"canary_idx shape: {canary_idx.shape}")
        #     # print(f"T shape: {T.shape}, v_adaptive: {v_adaptive}")
        # else:
        #     sorted_scores = torch.tensor(np.load(f"{output_dir}/sorted_scores.npy"))
...
def _run_train(
    ...
    with run:
        ...
        print("Predicting logits and evaluating full training data")
        full_train_data = data_generator.build_full_train_data(canaries_dir=canaries_dir)
        #train_data_full_pipe_with_label = full_train_data_with_label.build_datapipe()
        train_data_full_pipe = full_train_data.as_unlabeled().build_datapipe()
...
def parse_args() -> argparse.Namespace:
    ...
    # General args
    parser.add_argument("--lambdas", type=float, default=0.1, help="Use to optimize MIA")
    parser.add_argument("--sigma", type=float, default=0.1, help="Use to optimize MIA")
    ...
    attack_parser.add_argument("--global-variance", action="store_true", help="Use global variance for LiRA")
    #attack_parser.add_argument("--beta", type=float, default=0.1, help="Use to optimize MIA")
